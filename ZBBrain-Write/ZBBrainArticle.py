...
优化历史：
- v3.6.54 (当前版本 - 运行效率大幅优化):
    * 【优化1】metaso导航直接使用domcontentloaded，跳过90s networkidle超时（节省~90s/篇）
    * 【优化2】输入框检测selector超时从10s降为3s（节省~84s最坏情况）
    * 【优化3】页面渲染等待从5s降为2s，就绪检测间隔从3s降为1s（节省~6s/篇）
    * 【优化4】关键词间隔从5分钟降为2分钟（节省3min/关键词）
    * 【优化5】API速率限制等待从30s降为15s（节省15s）
    * 【优化6】浏览器关闭超时从30s降为10s（节省~60s/篇）
    * 【预计】单篇文章节省4-7分钟，整体效率提升25-35%
- v3.6.53 (运行间隔统一优化):
    * 【优化】统一所有运行间隔为5分钟（白天模式、晚上模式、定时检查）
    * 【修改】每日定时模式检查间隔从10分钟改为5分钟
- v3.6.52 (标题结构增强&目标公众号优化):
    * 【优化1】增强标题结构智能分析：优化提示词，添加文章结构模板
    * 【优化2】Fallback标题生成：基于规则添加标题，确保文章层次分明
    * 【优化3】重置公众号轮换索引到0，确保下次推送到"总包之声"
    * 【重要】确保每篇文章至少有3个二级标题，逻辑更清晰
- v3.6.51 (4大关键质量点优化):
    * 【修复1】标题提示词长度要求与35-45字要求一致（原18-28字已过时）
    * 【修复2】添加吸睛/标题党用语检测机制（30+词汇，扣分机制）
    * 【修复3】确保正文不重复热点问题（开头去重机制）
    * 【修复4】提高AI排版质量阈值到0.90（原0.85），确保更专业排版
- v3.6.50 (标题中文字符修复):
    * 【CRITICAL修复】emoji_pattern中的\U000024C2-\U0001F251范围过于宽泛，覆盖了中文字符！
    * 【修复】将宽泛范围拆分为精确的带圈字符范围：
      - \U000024C2-\U000024FF (Enclosed Alphanumerics)
      - \U0001F100-\U0001F1FF (Enclosed Alphanumeric Supplement)
      - \U0001F200-\U0001F24F (Enclosed Ideographic Supplement)
    * 【影响】标题现在能正确保留中文字符，不再被误删除
- v3.6.49 (广告图重试修复):
    * 【CRITICAL修复】修复_answer_to_markdown重试路径中enable_bottom_ad_ad拼写错误
    * 【验证】确认公众号轮换机制正常工作
    * 【验证】确认广告图防重复机制正常工作（inserted_ad_urls追踪）
- v3.6.48 (关键词轮换修复):
    * 【CRITICAL修复】添加缺失的get_topic_keywords()方法
    * 【CRITICAL修复】晚上模式现在使用关键词轮换机制，从上次停止位置继续
    * 【CRITICAL修复】每日定时模式现在也使用关键词轮换机制
    * 【重要】确保所有运行模式（单次/晚上/定时）都正确使用轮换状态
- v3.6.47 (Super-Skill全面优化):
    * 【CRITICAL修复】提高MD结构化max_tokens (10000→16000)
    * 【CRITICAL修复】提高MD验证覆盖率阈值 (80%→90%)
    * 【CRITICAL修复】提高占位符清理后质量阈值 (70%→85%)
    * 【CRITICAL修复】增强长回答完整性验证（1500+字符也需检查标记）
    * 【重要】基于Super-Skill 14阶段开发流程进行全面审计和优化
- v3.6.46 (标题特殊字符过滤):
    * 【CRITICAL修复】标题生成时自动移除emoji表情和特殊字符
    * 【CRITICAL修复】创建草稿前再次验证并清理标题
    * 【重要】只允许中文、英文、数字、基本标点
- v3.6.45 (排版质量保障强化):
    * 【CRITICAL修复】质量评分低于0.85时强制使用内置专业排版方法
    * 【CRITICAL修复】_simple_convert异常fallback现在有基本Markdown转HTML排版
    * 【重要】确保所有路径都经过排版处理，永不返回原始Markdown
- v3.6.44 (内容完整性三重保障):
    * 【CRITICAL修复】新增AI截断消息检测 - 检测"由于篇幅限制"等12种截断模式
    * 【CRITICAL修复】提高HTML质量阈值 - 从85%/70%提高到95%/85%
    * 【CRITICAL修复】新增推送前最终完整性验证 - 确保不完整内容永不推送
    * 【CRITICAL修复】新增MD保存前完整性验证 - 检测标记、长度、覆盖率
    * 【重要】三重保障：MD验证 → HTML验证 → 推送前验证
- v3.6.43 (baoyu-skills专业排版整合):
    * 【MAJOR】整合baoyu-skills项目的微信公众号排版最佳实践
    * 【MAJOR】新增专业级主题配色系统（融合baoyu-skills 12种颜色预设）
    * 【MAJOR】优化CSS样式：药丸形H1标题、底部边框H2、左侧边框H3、圆角容器
    * 【MAJOR】优化字体栈：使用baoyu推荐的中文字体栈确保跨平台一致性
    * 【MAJOR】优化行高和字间距：2.0行高、0px字间距（baoyu modern风格）
    * 【MAJOR】优化引用块样式：7px粗左边框、10px圆角、半透明背景
    * 【MAJOR】优化代码块样式：10px圆角、内阴影效果、专业等宽字体
    * 【重要】学习来源：https://github.com/JimLiu/baoyu-skills
- v3.6.42 (专业级微信公众号排版修复):
    * 【CRITICAL修复】增强_simple_convert方法，添加专业级微信公众号CSS样式
    * 【CRITICAL修复】支持7种主题颜色（秋日暖光、春日清新、深海静谧、优雅金、活力红、简约蓝、专注绿）
    * 【CRITICAL修复】优化Markdown转HTML处理，支持标题、段落、引用、列表等专业样式
    * 【CRITICAL修复】移除AI提示词中的占位符图片指令，改为绝对禁止添加任何图片
    * 【CRITICAL修复】移除结尾签名模板，与"绝对禁止添加签名"规则保持一致
    * 【重要】解决当AI转换质量评分为0时，fallback方法生成无样式HTML的问题
- v3.6.41 (标题字数一致性修复):
    * 【CRITICAL修复】标题生成重试提示词中的字数要求从28-30更新为35-45
    * 【CRITICAL修复】关键词智能插入时的长度检查从32字更新为45字
    * 【重要】确保所有硬编码的字数限制与config.ini中的35-45字要求一致
- v3.6.40 (标题字数扩展):
    * 【字数要求调整】标题从26-30字扩展到35-45字，更充分表达内容
    * 【验证逻辑更新】过短阈值从<10改为<25，超长阈值从>32改为>47
    * 【评分标准更新】最佳长度35-45字(30分)，次优32-48字(25分)
- v3.6.39 (标题精准优化):
    * 【标题冗余修复】"EPC"和"总承包"二选一，不再同时出现如"EPC总包"
    * 【场景具体化】标题必须基于热点问题的具体场景（如"松辽委严查"、"2026安全生产"）
    * 【禁止通用标题】标题必须体现问题的具体事件/政策/主体，不能泛泛而谈
- v3.6.38 (智能场景检测):
    * 【场景化优化】不生套格式，根据问题实际场景智能添加背景
    * 【判断问题是否已包含具体场景】避免不必要的转换
    * 【回复完成判断】添加基于时间的2分钟无增长检测
    * 【重要】目的是表达清楚用户真问题的具体场景
- v3.6.37 (Super-Skill全面优化):
    * 【要求1优化】智能场景化句式转换 - 根据问题类型选择最合适的模板
    * 【要求2修复】移除标题硬截断逻辑 - 标题必须是完整句子，超长则触发重新生成
    * 【要求3强化】正文完整性验证阈值从80%提高到90%，低于95%警告
    * 【要求4实现】广告图3次重试失败后直接删除，不再尝试本地路径备选
    * 【重要】禁止任何形式的硬截断，确保标题和正文100%完整
- v3.6.36 (精简占位符检测):
    * 【优化】精简占位符检测模式，避免过度检测误判正常内容
    * 【移除】移除过于宽泛的模式（省略...、图片占位等）
    * 【保留】只保留真正表示AI生成不完整的核心模式
    * 【重要】平衡检测严格度，减少误报
- v3.6.35 (Super-Skill全面优化):
    * 【CRITICAL修复】验证异常返回0.0而非0.8，强制重试
    * 【CRITICAL修复】修复裸except语句（2处）
    * 【HIGH修复】清理占位符后二次验证
    * 【HIGH修复】扩展占位符模式列表（12+新增模式）
    * 【重要】采用"检测+清理+二次验证"三重保障机制
- v3.6.34 (占位符图片严格清理):
    * 【v3.6.34新增】CRITICAL: 最后防线 - 无论质量评分如何，都清理占位符图片
    * 【v3.6.34新增】遍历所有img标签，检测并移除包含占位符关键词的标签
    * 【v3.6.34新增】清理空的图片包装div和占位符注释
    * 【重要】采用"检测+清理"双重保障机制
- v3.6.33 (占位符图片禁止):
    * 【v3.6.33新增】CRITICAL: 占位符图片检测 - 检测到"图片URL"、"描述"等占位符直接返回0分
    * 【v3.6.33新增】AI提示词强化 - 禁止添加任何占位符图片标签
    * 【重要教训】AI生成HTML时可能添加占位符图片标签，必须从提示词和验证两端禁止
- v3.6.32 (HTML完整性强制验证):
    * 【v3.6.32新增】CRITICAL: 占位符注释检测 - 检测到"More content follows"等占位符直接返回0分
    * 【v3.6.32新增】CRITICAL: 总包大脑完整性标记检测 - 必须包含"总包学园"等标记
    * 【v3.6.32新增】CRITICAL: 内容长度验证 - HTML内容长度必须≥原文80%
    * 【重要教训】AI生成HTML时可能产生占位符注释导致内容截断和广告图重复插入
- v3.6.31 (质量强化):
    * 【热点问题】强制验证场景化句式，不符合则自动转换
    * 【标题生成】双关键词强制验证（搜索关键词+"EPC/总承包"）
    * 【标题完整性】必须是完整句子，智能截断保持语义
    * 【内容完整性-硬性指标】100%完整保留总包大脑回复，绝不做任何删减
    * 【完整性检测优化】检测到"总包学园"标记即100%完整
    * 【最小长度优化】降低MIN_VALID_ANSWER_LENGTH从500到200
    * 【长度限制移除】移除所有可能导致内容截断的长度限制
    * 【超时保护优化】即使超时也返回已获取的内容，不返回None
    * 【广告图片】防重复插入 + 3次重试失败后自动删除
- v3.6.30 (性能精简):
    * 移除思考模式选择：总包大脑默认"快思考"，无需切换
    * 减少页面交互步骤，提升执行效率
- v3.6.29 (性能优化):
    * API连接池：单例模式减少连接开销
    * 智能重试：指数退避策略 + 错误类型识别
    * 异常处理：分类处理速率限制/认证/超时错误
- v3.6.28 (质量全面提升):
    * 热点问题生成优化：强制使用场景化句式（"当……的时候"、"在遇到……的情况下"）
    * 标题生成优化：强制同时包含搜索关键词 + "EPC"或"总承包"字眼
    * 标题完整性保障：必须是完整句子，不能被截断
    * 广告图片优化：防止重复插入 + 3次重试失败后自动删除
    * 企业微信通知：新增配置更新通知功能
    * 【v3.6.29优化】API连接池：单例模式减少连接开销
    * 【v3.6.29优化】智能重试：指数退避策略 + 错误类型识别
    * 【v3.6.29优化】异常处理：分类处理速率限制/认证/超时错误
- v3.6.26 (每日定时模式):
    - 新增 --daily-19 参数：每天19:00自动启动晚上模式
    - 新增 run_daily_night_scheduler() 函数实现每日定时调度
    - 启动时发送企业微信通知
    - 每篇文章完成后发送企业微信通知
    - 每日运行完成后发送总结通知
    - 支持 --daily-hour 参数自定义启动时间
- v3.6.25 (广告语完整保留):
    - 重构内容提取逻辑，确保总包大脑回复的广告语完整保留
    - 新增策略A：优先查找包含"总包学园"关键词的完整回答容器
    - 移除最小长度过滤（100/200字符），避免截断短内容
    - 新增 containsAdKeywords() 函数检测广告语关键词
    - 三级策略确保内容完整性：广告语检测 → 传统提取 → 广泛搜索
- v3.6.19 (精准回复提取优化):
    * 新增排除关键词列表，过滤侧边栏内容
    * 优化JavaScript提取策略，支持边界检测
    * 排除"相关事件"、"相关组织"、"相关人物"等非回答内容
    * 实现多策略精准提取，确保100%只获取AI回答核心内容
  * 新增晚上模式：使用 --night 参数启动
  * 运行规则：依次使用所有关键词生成文章
  * 间隔时间：每个关键词完成后间隔5分钟
  * 停止条件：关键词用完

- v3.6.16 (快思考模式优化):
  * 思考模式调整：默认使用快思考模式，响应更快
  * 配置修复：fallback 默认值从 True 改为 False
  * 配置说明更新：config.ini 中明确说明默认为快思考模式
...
class ContentQualityScorer:
    ...
    # 【v3.6.51新增】吸睛/标题党用语检测 - 这些词汇应该避免
    CLICKBAIT_WORDS = [
        '震惊', '惊呆', '吓傻', '不敢相信', '无法想象', '不可思议',
        '竟然', '居然', '意想不到', '出人意料',
        '90%的人', '99%的人', '所有人都', '没人知道', '终于揭秘',
        '必看', '不看后悔', '速看', '马上删', '限时',
        '大揭秘', '曝光', '爆料', '内幕', '真相大白',
        '疯了', '疯了', '疯了', '泪奔', '哭晕', '笑喷',
        '涨知识了', '长见识了', '原来如此', '恍然大悟',
    ...
    def score_title(self, title: str, keyword: str = None) -> dict:
        ...
        # 1. 长度检查 (30分) - 【v3.6.40更新】35-45字要求
        if 35 <= len(title) <= 45:
            score += 30
        elif 32 <= len(title) <= 48:
            score += 25
        elif 28 <= len(title) <= 50:
            score += 15
            issues.append(f"标题长度({len(title)}字)建议在35-45字之间")
        else:
            score += 5
            issues.append(f"标题长度({len(title)}字)不符合要求(35-45字)")
        ...
        # 【v3.6.51新增】吸睛用语检测 - 扣分机制
        clickbait_found = [w for w in self.CLICKBAIT_WORDS if w in title]
        if clickbait_found:
            score -= 10  # 每个吸睛词汇扣10分
            issues.append(f"⚠️ 标题包含吸睛用语: {clickbait_found}，建议避免标题党表达")
...
class Config:
    ...
    def load_config(self) -> None:
        ...
        self.run_frequency_minutes = self.config.getint('定时任务', '运行间隔分钟', fallback=60)
...
{md_content}'''
    ...
    def get_topic_keywords(self) -> list:
        """【v3.6.48新增】获取所有关键词列表（用于批量运行模式）

        Returns:
            list: 关键词列表
        """
        if not hasattr(self, 'keywords_list') or not self.keywords_list:
            self._init_keyword_rotation()
        return self.keywords_list.copy() if self.keywords_list else [self.default_keyword]
...
class StagehandBrowser:
    ...
    async def close(self, save_cookies: bool = True, cookie_domain_filter: str = None) -> None:
        ...
        close_timeout = 10  # 【v3.6.54优化】从30s降为10s
...
class ZhipuAIAnalyzer:
    ...
    def __init__(self, config: Config, logger: Logger) -> None:
        ...
        # 【v3.6.29优化】使用连接池
        if ZhipuAIAnalyzer._connection_pool is None:
            ZhipuAIAnalyzer._connection_pool = APIConnectionPool(max_retries=3, backoff_factor=0.5)

        self.client = self._connection_pool.get_client(config.zhipu_api_key)
    ...
    def generate_hot_question(self, articles: List[Dict]) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 【v3.6.29优化】根据错误类型采取不同策略
                if 'RateLimit' in error_type or '429' in error_msg or 'rate limit' in error_msg.lower():
                    self.logger.warning(f"API速率限制 (尝试 {attempt + 1}/{max_retries})，等待15秒后重试...")
                    import time
                    time.sleep(15)  # 【v3.6.54优化】从30s降为15s
                elif 'Auth' in error_type or '401' in error_msg or '403' in error_msg:
                    self.logger.error(f"API认证失败: {error_msg}")
                    raise  # 认证错误不重试
                elif 'timeout' in error_msg.lower() or 'Timeout' in error_type:
                    self.logger.warning(f"API超时 (尝试 {attempt + 1}/{max_retries}): {error_msg}")
                    import time
                    time.sleep(2)  # 【v3.6.54优化】从5s降为2s
                else:
                    self.logger.error(f"智谱AI调用失败 (尝试 {attempt + 1}/{max_retries}): [{error_type}] {error_msg}")
                    if attempt < max_retries - 1:
                        import time
    ...
    def _validate_question(self, question: str) -> str:
        ...
        scenario_patterns = [
            ...
            ('面对', '时'),         # "面对……时，……"
            ('在', '过程中,当'),     # "在……过程中，当……"
            ('在', '过程中,当'),     # "在……过程中，当……"（逗号变体）
        ]

        has_scenario_pattern = False
        for start, end in scenario_patterns:
            if start in question:
                # 检查是否有对应的结束词
                end_variations = [end, end.replace(',', '，'), end.replace('，', ',')]
                for end_var in end_variations:
                    if end_var in question:
                        # 确保开始词在结束词之前
                        start_idx = question.find(start)
                        end_idx = question.find(end_var)
                        if start_idx < end_idx:
                            has_scenario_pattern = True
                            break
                if has_scenario_pattern:
                    break

        # 【v3.6.38重构】如果不符合场景化句式，智能转换为场景化句式
        # 核心原则：不生套格式，目的是表达清楚用户真问题的具体场景
        if not has_scenario_pattern:
            self.logger.warning(f"问题不符合场景化句式: {question[:50]}...")

            # 提取问题的核心内容
            core_question = question.rstrip('。！？，,.!?')

            # 【v3.6.38核心改进】先判断问题是否已经足够具体
            # 如果问题已经包含具体场景描述（项目类型、阶段、角色等），则不需要强制转换
            specific_indicators = ['项目', '工程', '合同', '施工', '设计', '采购', '验收', '结算',
                                  '投标', '招标', '变更', '索赔', '风险', '质量', '安全', '进度',
                                  '分包', '业主', '监理', '总包', '承包商']
            has_specific_context = any(indicator in core_question for indicator in specific_indicators)

            if has_specific_context:
                # 问题已包含具体场景，只需要确保问句格式正确
                if not core_question.endswith('？') and not core_question.endswith('?'):
                    question = core_question + '，应如何应对？'
                else:
                    question = core_question
                self.logger.info(f"✓ 问题已包含具体场景，保持原意: {question[:60]}...")
            else:
                # 问题过于抽象，尝试添加场景背景
                # 【v3.6.38关键】不要生套格式，根据问题内容智能添加最合适的场景

                # 检测问题中的关键词，推断最可能的项目阶段/场景
                if any(kw in core_question for kw in ['变更', '修改', '调整']):
                    # 设计/施工变更场景
                    question = f"在EPC项目实施过程中遇到{core_question.rstrip('？?')}，应如何处理？"
                elif any(kw in core_question for kw in ['风险', '隐患', '问题']):
                    # 风险管理场景
                    question = f"面对{core_question.rstrip('？?')}时，EPC总承包商应如何防控？"
                elif any(kw in core_question for kw in ['合同', '条款', '约定']):
                    # 合同管理场景
                    question = f"在EPC合同执行中{core_question.rstrip('？?')}，应如何应对？"
                elif any(kw in core_question for kw in ['成本', '费用', '预算', '造价']):
                    # 成本控制场景
                    question = f"在EPC项目成本管理中{core_question.rstrip('？?')}，应如何控制？"
                elif 'EPC' in core_question or '总承包' in core_question:
                    # 已有EPC关键词，保持原意基础上优化表达
                    if '，' in core_question:
                        question = core_question if core_question.endswith('？') or core_question.endswith('?') else core_question + '？'
                    else:
                        question = f"{core_question.rstrip('？?')}，应如何应对？"
                else:
                    # 通用场景：添加EPC项目背景
                    question = f"在EPC总承包项目中{core_question.rstrip('？?')}，应如何处理？"

                self.logger.info(f"✓ 已智能添加场景背景: {question[:60]}...")
    ...
    def generate_catchy_title(self, question: str, answer: str, keyword: str = None) -> str:
        ...
        # 备用标题模板（仅在AI生成失败时使用）- 紧扣具体场景、包含感叹号（35-45字）【v3.6.51优化】移除吸睛用语
        title_templates = [
            "水利工程EPC设计变更索赔这3个要点必须掌握！规避风险很重要！",
            "水利EPC项目设计变更风险如何规避？这4个策略非常实用！",
            "水利EPC分包商违约怎么办？这5个合同条款必须提前看清！",
            "水利EPC项目成本控制这5个关键点！提升项目管理水平！",
            "水利EPC项目利润提升30%的4个实战技巧！项目管理必读！",
            "水利EPC合同这3个核心条款必须看清！规避履约风险！",
            "水利EPC项目风险管理这3个关键点必须掌握！保障项目顺利！",
            "水利EPC项目工期延误这4个应对策略！保障项目按期交付！",
        ]

        def _clean_title(raw_title: str) -> str:
            """清理标题的辅助函数

            【v3.6.46增强】移除微信公众号不允许的特殊字符
            - 移除emoji表情
            - 移除特殊符号
            - 只保留中文、英文、数字、基本标点
            """
            ...
            # 【v3.6.46新增】移除emoji表情和特殊字符
            # 微信公众号标题允许的字符：中文、英文、数字、基本标点
            # 不允许：emoji、特殊符号、控制字符

            # 方法1：使用正则移除emoji（常见emoji范围）
            emoji_pattern = re.compile(
                ...
                "\U00002702-\U000027B0"
                # 【v3.6.50修复】移除过于宽泛的范围 \U000024C2-\U0001F251（覆盖了中文字符！）
                # 改用精确的带圈字符范围
                "\U000024C2-\U000024FF"  # Enclosed Alphanumerics (仅60个字符)
                "\U0001F100-\U0001F1FF"  # Enclosed Alphanumeric Supplement
                "\U0001F200-\U0001F24F"  # Enclosed Ideographic Supplement (不含U+1F251之后)
...
标题必须包含搜索关键词「{keyword}」，这是文章的核心主题，不能遗漏！"""
            ...
            try:
                ...
                if last_error and retry > 0:
                    if "超长" in str(last_error):
                        retry_hint = "\n\n【紧急提醒】上次生成的标题太长了！这次必须严格控制在35-45字以内，超过45字直接拒绝！"
                    elif "过短" in str(last_error):
                        retry_hint = "\n\n【紧急提醒】上次生成的标题太短了！这次必须生成35-45字的完整标题。"
                    elif "空" in str(last_error) or "None" in str(last_error):
                        retry_hint = "\n\n【紧急提醒】上次没有正确输出！这次必须直接输出一个完整的标题，不要任何解释。"
                    else:
                        retry_hint = "\n\n【紧急提醒】上次生成失败，这次请务必严格按照要求生成35-45字的完整标题。"
                ...
                # 【v3.6.39新增】检查EPC与总承包冗余 - 两者只需其一
                if 'EPC' in title and ('总承包' in title or '总包' in title):
                    self.logger.warning(f"⚠️ 标题存在冗余（同时包含EPC和总承包/总包），尝试修复")
                    # 优先保留EPC，移除总承包/总包
                    if 'EPC总承包' in title:
                        title = title.replace('EPC总承包', 'EPC')
                        self.logger.info(f"✓ 移除冗余: EPC总承包 → EPC")
                    elif 'EPC总包' in title:
                        title = title.replace('EPC总包', 'EPC')
                        self.logger.info(f"✓ 移除冗余: EPC总包 → EPC")
                    elif '总承包EPC' in title:
                        title = title.replace('总承包EPC', 'EPC')
                        self.logger.info(f"✓ 移除冗余: 总承包EPC → EPC")
                    self.logger.info(f"修复后标题: {title}")
                ...
                # 【v3.6.40优化】超长标题智能处理 - 优先智能截断，而非硬重试（目标35-45字）
                if len(title) > 47:  # 超过47字视为超长
                    # 检查是否是完整句子
                    if _is_complete_sentence(title):
                        # 是完整句子，尝试智能截断到45字
                        truncated = _smart_truncate_title(title, 45)
                        if len(truncated) <= 45 and _is_complete_sentence(truncated):
        ...
        # 检查标题长度（只记录警告，不截断）- 【v3.6.40更新】35-45字要求
        if len(title) < 35:
            self.logger.warning(f"生成的标题略短（{len(title)}字），建议35-45字")

        # 【v3.6.10】移除硬截断逻辑 - 标题必须是完整句子
        # 如果标题超长，说明上面重试失败后使用了备用模板，备用模板都是合适长度的

        # 【v3.6.39新增】检查标题是否包含热点问题的具体场景
        # 从问题中提取关键实体/事件（排除通用词）
        generic_words = {'怎么', '如何', '什么', '为什么', '哪些', '怎样', '应该', '需要', '可以', '能够',
                        '作为', '进行', '确保', '避免', '应对', '解决', '处理', '管理', '问题', '工作',
                        '情况', '背景下', '过程中', '情况下', '的时候', '如何应对', '应如何'}
        # 提取问题中的关键词（2-6字的词组）
        import re
        question_key_terms = set()
        # 匹配中文词组（2-6字）
        for match in re.finditer(r'[\u4e00-\u9fa5]{2,6}', question):
            term = match.group()
            if term not in generic_words and len(term) >= 2:
                question_key_terms.add(term)

        # 检查标题是否包含至少一个问题的关键词
        title_has_specific = False
        matched_terms = []
        for term in question_key_terms:
            if term in title:
                title_has_specific = True
                matched_terms.append(term)

        if not title_has_specific and question_key_terms:
            self.logger.warning(f"⚠️ 标题未包含热点问题的具体场景词！")
            self.logger.warning(f"   问题场景词: {list(question_key_terms)[:10]}...")
            self.logger.warning(f"   标题: {title}")
        else:
            self.logger.info(f"✓ 标题包含场景词: {matched_terms}")

        # 【v3.6.39重构】关键词验证：搜索关键词 + EPC或总承包（二选一，不冗余）
        has_keyword = not keyword or keyword in title
        has_epc = 'EPC' in title or '总承包' in title

        if not has_keyword or not has_epc:
            self.logger.warning(f"标题缺少必要关键词 - 搜索词: {'✓' if has_keyword else '✗'}, EPC/总承包: {'✓' if has_epc else '✗'}")

            # 【v3.6.37重要】禁止硬截断！必须保持句子完整性
            # 尝试智能添加缺失的关键词
            if not has_keyword and keyword:
                # 先尝试在EPC前插入关键词
                if 'EPC' in title:
                    new_title = title.replace('EPC', f'{keyword}EPC', 1)
                    if len(new_title) <= 45:  # 【v3.6.41修复】与35-45字要求一致
                        title = new_title
                        self.logger.info(f"✓ 在EPC前插入关键词「{keyword}」: {title}")
                    else:
                        # 【v3.6.37修复】不在超长时截断，而是重新生成更短的标题
                        self.logger.warning(f"插入关键词后标题过长({len(new_title)}字)，触发重新生成")
                        # 标记需要重新生成
                        title = ""
                elif '总承包' in title:
                    new_title = title.replace('总承包', f'{keyword}总承包', 1)
                    if len(new_title) <= 45:
                        title = new_title
                        self.logger.info(f"✓ 在总承包前插入关键词「{keyword}」: {title}")
                    else:
                        self.logger.warning(f"插入关键词后标题过长({len(new_title)}字)，触发重新生成")
                        title = ""
                else:
                    # 都没有，在开头添加
                    new_title = f"{keyword}EPC{title}"
                    if len(new_title) <= 45:
                        title = new_title
                        self.logger.info(f"✓ 添加关键词+EPC: {title}")
                    else:
                        self.logger.warning(f"添加关键词后标题过长({len(new_title)}字)，触发重新生成")
                        title = ""

            # 再次检查是否有EPC/总承包
            if title and 'EPC' not in title and '总承包' not in title:
                # 尝试在合适位置插入
                if keyword and keyword in title:
                    # 在关键词后添加EPC
                    new_title = title.replace(keyword, f'{keyword}EPC', 1)
                    if len(new_title) <= 45:  # 【v3.6.41修复】与35-45字要求一致
                        title = new_title
                        self.logger.info(f"✓ 添加EPC: {title}")
                    else:
                        self.logger.warning(f"添加EPC后标题过长({len(new_title)}字)，触发重新生成")
                        title = ""
                else:
                    # 在开头添加
                    new_title = f"EPC{title}"
                    if len(new_title) <= 45:
                        title = new_title
                        self.logger.info(f"✓ 添加EPC前缀: {title}")
                    else:
                        self.logger.warning(f"添加EPC后标题过长({len(new_title)}字)，使用备用模板")
                        title = random.choice(title_templates) if 'title_templates' in dir() else f"{keyword}EPC项目实战要点解析！"

        # 【v3.6.39最终检查】确保无EPC+总承包冗余
        if title and 'EPC' in title and ('总承包' in title or '总包' in title):
            self.logger.warning(f"⚠️ 最终检查发现冗余，修复中...")
            if 'EPC总承包' in title:
                title = title.replace('EPC总承包', 'EPC')
            elif 'EPC总包' in title:
                title = title.replace('EPC总包', 'EPC')
                ...
                title = title.replace('总承包EPC', 'EPC')
            self.logger.info(f"✓ 冗余修复后: {title}")
...
class MetasoAutomation:
    ...
    async def _click_settings_gear_button(self) -> bool:
        ...
        【v3.6.18优化】增加备用检测逻辑，当找不到"总包大脑"文本时也能工作
        """
        ...
        try:
            ...
            settings_info = await self.browser.page.evaluate('''() => {
                ...
                // 【v3.6.18新增】扩展关键词列表
                const keywords = ['总包行业大脑', '总包大脑', '总包', '秘塔', 'AI搜索', '大脑'];
                ...
                // 【v3.6.18新增】如果还是找不到，使用页面宽度作为参考
                if (zongbaoRight === 0) {
                    zongbaoRight = Math.round(window.innerWidth * 0.3); // 假设标题占30%宽度
                }

                // 步骤2: 在右侧查找齿轮按钮
                for (const btn of buttons) {
                    const rect = btn.getBoundingClientRect();
                    if (rect.top > 100 || rect.width === 0 || rect.height === 0) continue;
                    ...
                    const candidate = {
                        ...
                        isRightOfZongbao: rect.left > zongbaoRight && rect.left < zongbaoRight + 200
                    };
                    ...
                    if (viewBox === '0 0 20 20' && pathCount === 2 && !btnText) {
                        ...
                        // 【v3.6.18新增】如果没有找到，选择任何符合条件的齿轮按钮
                        if (!bestMatch && path0 > 100 && path1 > 20) {
                            bestMatch = candidate;
                        }
                    }
                }
    ...
    async def _navigate_to_metaso(self):
        ...
        try:
            ...
            # 【v3.6.54优化】直接使用domcontentloaded，跳过90s networkidle超时
            await self.browser.goto(self.config.metaso_url, wait_until="domcontentloaded", timeout=30000)

            # 【v3.6.54优化】减少初始等待时间从5s到2s
            ...
            # 【v3.6.18新增】检测页面是否真正加载完成
            for i in range(3):
                try:
                    # 检测关键元素是否存在
                    page_ready = await self.browser.page.evaluate('''() => {
                        // 检测多种可能的页面元素
                        const checks = {
                            hasTitle: document.body.innerText.includes('总包') ||
                                     document.body.innerText.includes('大脑') ||
                                     document.body.innerText.includes('秘塔'),
                            hasInput: document.querySelector('textarea') !== null ||
                                     document.querySelector('[contenteditable="true"]') !== null ||
                                     document.querySelector('input[type="text"]') !== null,
                            hasContent: document.body.innerText.length > 100
                        };
                        return checks.hasTitle || checks.hasInput || checks.hasContent;
                    }''')

                    if page_ready:
                        self.logger.info(f"✓ 页面加载检测通过 (尝试 {i+1}/3)")
                        break
                    else:
                        self.logger.warning(f"页面内容检测未通过，等待... (尝试 {i+1}/3)")
                        await asyncio.sleep(1)  # 【v3.6.54优化】从3s降为1s
                except Exception as e:
                    self.logger.warning(f"页面检测出错: {e}")
                    await asyncio.sleep(1)  # 【v3.6.54优化】从2s降为1s
...
_总包大脑自动写作系统 v3.6.8_"""
    ...
    async def _capture_baseline_content(self):
        """【v3.6.20新增】捕获发送问题前的页面静态内容作为基线
        ...
        try:
            baseline_script = """(() => {
                ...
                // 提取纯文本，用特殊分隔符连接
                return allText.map(item => item.text).join('|||');
            })()"""

            self._baseline_content = await self.browser.page.evaluate(baseline_script)
            self.logger.info(f"✓ 已捕获页面基线内容，长度: {len(self._baseline_content or '')} 字符")

        except Exception as e:
            self.logger.warning(f"捕获基线内容失败: {e}")
            self._baseline_content = ""

    async def _send_question(self, question: str):
        """使用Stagehand发送问题到总包大脑

        【v3.6.30优化】移除思考模式选择，使用总包大脑默认的"快思考"模式
        【v3.6.20新增】在发送问题前捕获页面静态内容作为基线
        """
        self.logger.info(f"发送问题: {question}")

        try:
            # 【v3.6.20新增】捕获发送前的页面静态内容作为基线
            await self._capture_baseline_content()

            # 【v3.6.30优化】总包大脑默认就是"快思考"模式，无需主动切换
            self.logger.info("使用总包大脑默认的快思考模式")

            self.logger.info("=" * 50)
            self.logger.info("步骤1：发送问题到总包大脑")
            self.logger.info("=" * 50)
    ...
    async def _select_thinking_mode(self, mode: str = "快思考") -> bool:
        """选择思考模式 - 通用方法

        【v3.6.21新增】支持选择"快思考"或"长思考"模式
        【v3.6.21优化】先检查当前模式，如果已经是目标模式则跳过
        ...
        """
        try:
            self.logger.info(f"检查/切换思考模式到: {mode}")

            # 等待页面稳定
            await asyncio.sleep(1)

            # 【v3.6.21优化】先检查当前模式是否已经是目标模式
            current_mode = await self._get_current_thinking_mode()
            if current_mode == mode:
                self.logger.info(f"✓ 当前已经是 {mode} 模式，无需切换")
                return True

            self.logger.info(f"当前模式: {current_mode or '未知'}，需要切换到: {mode}")

            # 找到模式选择器
            mode_selector_keywords = ['快思考', '长思考']
            current_mode_element = None

            for keyword in mode_selector_keywords:
                try:
                    locator = self.browser.page.locator(f'span:has-text("{keyword}")').first
                    if await locator.count() > 0 and await locator.is_visible():
                        current_mode_element = locator
                        self.logger.info(f"找到模式选择器: {keyword}")
                        break
                except Exception as e:
                    self.logger.debug(f"查找模式选择器失败: {keyword}, 错误: {e}")
                    continue

            if not current_mode_element:
                self.logger.warning("未找到模式选择器，尝试直接查找目标模式")
                return await self._click_mode_option(mode)

            # 悬停触发下拉菜单
            self.logger.info("悬停在模式选择器上以触发下拉菜单...")
            await current_mode_element.hover(timeout=5000)
            await asyncio.sleep(0.8)  # 等待下拉菜单动画

            # 点击目标模式
            return await self._click_mode_option(mode)

        except Exception as e:
            self.logger.warning(f"切换到{mode}失败: {str(e)}")
            return False

    async def _get_current_thinking_mode(self) -> Optional[str]:
        """获取当前思考模式

        【v3.6.21新增】检测页面当前显示的是"快思考"还是"长思考"
        """
        try:
            # 使用 JavaScript 检查当前显示的模式
            current_mode = await self.browser.page.evaluate('''() => {
                // 查找显示"快思考"或"长思考"的元素
                const spans = document.querySelectorAll('span, div, button');
                for (const el of spans) {
                    const text = (el.innerText || el.textContent || '').trim();
                    if (text === '快思考' || text === '长思考') {
                        // 检查是否可见且在模式选择区域
                        const rect = el.getBoundingClientRect();
            ...
            self.logger.debug(f"获取当前模式失败: {e}")
            return None

    async def _click_mode_option(self, mode: str) -> bool:
        """点击下拉菜单中的模式选项"""
        mode_selectors = [
            f'text={mode}',
            f'span:has-text("{mode}")',
            f'div:has-text("{mode}")',
            f'button:has-text("{mode}")',
            f'li:has-text("{mode}")',
            f'[class*="dropdown"] span:has-text("{mode}")',
            f'[class*="menu"] span:has-text("{mode}")',
        ]
        ...
        for selector in mode_selectors:
            try:
                ...
                if await locator.count() > 0 and await locator.is_visible():
                    ...
                    self.logger.info(f"✓ 成功点击{mode}选项: {selector}")
                    await asyncio.sleep(0.5)
                    return True
            except Exception as e:
        ...
        self.logger.warning(f"未找到{mode}选项")
        return False
            ...
            for selector in standard_selectors:
                ...
                try:
                    ...
                    element = await self.browser.page.wait_for_selector(
                        selector,
                        state='visible',
                        timeout=3000  # 【v3.6.54优化】3秒超时（原10秒）
                    )
    ...
    async def _wait_for_answer(self) -> Optional[str]:
        ...
        # 【v3.6.31优化】降低最小回答长度要求，避免误判短但完整的回复
        MIN_VALID_ANSWER_LENGTH = 200

        # 【v3.6.38新增】基于时间的内容增长监控
        NO_GROWTH_THRESHOLD_SECONDS = 120  # 2分钟内无增长视为完成
        last_growth_time = time.time()  # 记录最后一次内容增长的时间
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                if current_content:
                    ...
                    # 【v3.6.38优化】记录内容增长情况 - 添加基于时间的判断
                    if content_length > max_content_length:
                        growth = content_length - max_content_length
                        max_content_length = content_length
                        no_growth_count = 0
                        last_growth_time = time.time()  # 【v3.6.38新增】记录最后一次增长时间
                        self.logger.info(f"内容增长中... 当前长度: {content_length} 字符 (增长: +{growth})")
                    else:
                        no_growth_count += 1
                        # 【v3.6.38新增】计算无增长时间
                        no_growth_seconds = time.time() - last_growth_time
                        self.logger.debug(f"内容未增长: {no_growth_count} 次, 无增长时长: {no_growth_seconds:.0f}秒, 当前长度: {content_length} 字符")

                        # 【v3.6.38新增】2分钟内无增长视为完成
                        if no_growth_seconds >= NO_GROWTH_THRESHOLD_SECONDS:
                            content_growth_stopped = True
                            self.logger.info(f"⏱️ 内容已{no_growth_seconds:.0f}秒无增长，视为回答完成")
        ...
        for attempt in range(3):
            ...
            if final_content and len(final_content) >= MIN_VALID_ANSWER_LENGTH:
                ...
                # 【v3.6.31优化】即使长度不足，最后一次尝试也要返回内容
                if attempt < 2:
                    self.logger.warning(f"⚠️ 获取的内容长度不足 ({len(final_content)} 字符)，重试... (尝试 {attempt + 1}/3)")
                    continue
                else:
                    # 最后一次尝试，即使长度不足也返回
                    self.logger.warning(f"⚠️ 最终获取的内容长度不足 ({len(final_content)} 字符)，但仍返回以避免丢失")
                    return final_content
    ...
    def _is_answer_complete(self, answer: str) -> bool:
        """【v3.6.31强化】验证回答是否完整 - 确保总包大脑回复100%完整

        检查回答是否有完整的结尾标志

        【v3.6.31更新】
        - 降低最小长度限制到100字符，避免误判短但完整的回复
        - 总包学园标记检测是最高优先级，检测到直接返回True
        - 增加更多完整性判断条件
        """
        if not answer:
            return False

        # 【v3.6.31优化】降低最小长度限制，避免误判
        # 如果内容太短（<100字符），才认为不完整
        if len(answer) < 100:
            self.logger.warning(f"回答过短（{len(answer)}字符），可能不完整")
            return False

        # 检查是否有完整的结尾标志
        answer_stripped = answer.strip()

        # 【v3.6.31强化】最优先检查：总包大脑特定的完整性标记
        # 总包大脑回复的最后一定包含"总包学园"字样 - 这是100%完整的标志
        ZONGBAO_MARKERS = ['总包学园', 'epcschool', 'EPCSchool', '喝酒聊天']
        for marker in ZONGBAO_MARKERS:
            if marker in answer_stripped:
                self.logger.info(f"✅ 检测到完整性标记 '{marker}'，回答100%完整")
                return True

        # 1. 检查是否以完整的句子结尾（句号、感叹号、问号）
        if answer_stripped[-1] in '。！？.!?':
            self.logger.info(f"✅ 以完整句子结尾，回答完整")
            return True
        ...
        for pattern in ending_patterns:
            if pattern in answer_stripped[-100:]:  # 检查最后100个字符
                self.logger.info(f"✅ 检测到结尾词 '{pattern}'，回答完整")
                return True
        ...
        if len(paragraphs) >= 3:
            ...
            if last_para.endswith('。') or last_para.endswith('！') or last_para.endswith('？'):
                self.logger.info(f"✅ 多段落结构完整，回答完整")
                return True
            # 如果最后一个段落超过30字且不是以逗号结尾，也认为可能是完整的
            if len(last_para) >= 30 and not last_para.endswith('，'):
                self.logger.info(f"✅ 最后段落完整（{len(last_para)}字），回答完整")
                return True

        # 4. 【v3.6.47增强】如果回答超过1500字，仍需检查完整性标记
        # 不再直接返回True，而是检查是否有完整性标记
        if len(answer_stripped) >= 1500:
            # 检查是否有总包大脑完整性标记
            completeness_markers = ['总包学园', 'epcschool', 'EPCSchool', '喝酒聊天', '中国首所工程总承包线上学校']
            has_marker = any(marker in answer_stripped for marker in completeness_markers)
            if has_marker:
                self.logger.info(f"✅ 长回答且有完整性标记（{len(answer_stripped)}字符），回答完整")
                return True
            # 检查是否有明确的结尾标志
            ending_markers = ['希望对您有帮助', '如有需要请', '祝您工作顺利', '感谢您的提问', '欢迎继续提问']
            has_ending = any(marker in answer_stripped for marker in ending_markers)
            if has_ending:
                self.logger.info(f"✅ 长回答且有结尾标志（{len(answer_stripped)}字符），回答完整")
                return True
            # 即使长度足够但没有完整性标记，记录警告但允许通过（避免无限等待）
            self.logger.warning(f"⚠️ 长回答但无完整性标记（{len(answer_stripped)}字符），谨慎认为完整")
            return True

        # 5. 【v3.6.31新增】检查是否有"第X部分"结构
        if '第' in answer_stripped and '部分' in answer_stripped:
            # 如果有分部分结构，检查是否有结论部分
            conclusion_markers = ['结论', '总结', '建议', '行动', '清单']
            for marker in conclusion_markers:
                if marker in answer_stripped:
                    self.logger.info(f"✅ 检测到结构化内容+结论标记 '{marker}'，回答完整")
                    return True
    ...
    async def _extract_answer_with_js(self) -> Optional[str]:
        ...
        【v3.6.20重构】精准提取优化 - 不过度过滤：
        1. 获取AI回复的全部内容（不因中间出现关键词就排除）
        2. 只排除真正的侧边栏区域（基于DOM结构优先）
        3. 侧边栏标题检测改为精确匹配（不是包含匹配）
        4. 【v3.6.20新增】使用基线对比策略排除静态内容
        5. 保持AI回复完整性
        """
        try:
            # 【v3.6.20新增】策略0: 基线对比 - 提取动态生成的AI回复
            # 对比发送问题前后的页面内容，只获取新增的动态内容
            if hasattr(self, '_baseline_content') and self._baseline_content:
                diff_script = """((baselineContent) => {
                    // 获取当前页面所有文本内容
                    const currentTexts = [];
                    const walker = document.createTreeWalker(
                        document.body,
                    ...
                    // 分割基线内容
                    const baselineSet = new Set(baselineContent.split('|||'));

                    // 找出新增的内容（动态生成的AI回复）
                    const newTexts = [];
                    for (const item of currentTexts) {
                        if (!baselineSet.has(item.text)) {
                            newTexts.push(item);
                        }
                    }
                    ...
                    if (newTexts.length > 0) {
                        ...
                        // 合并文本，用换行连接
                        const mergedText = newTexts.map(item => item.text).join('\\n');
                        return mergedText;
                    }
                ...
                try:
                    dynamic_content = await self.browser.page.evaluate(
                        f"(() => {{ const baselineContent = {repr(self._baseline_content)}; return {diff_script.substring(13, diff_script.length-3)}; }})()"
                    )
                    # 简化：直接传递基线内容
                    diff_script_with_param = f"""(() => {{
                        const baselineContent = {repr(self._baseline_content)};
                        {diff_script.substring(diff_script.indexOf('{')+1, diff_script.rfind('}'))}
                    }})()"""
                    # 使用更简单的方式
                    dynamic_content = await self.browser.page.evaluate(
                        f"""(baseline) => {{
                            ...
                            const newTexts = currentTexts.filter(t => !baselineSet.has(t));
                            return newTexts.join('\\n');
                        }}""",
                        self._baseline_content
                    )
                    if dynamic_content and len(dynamic_content.strip()) > 100:
                        self.logger.info(f"✅ 使用基线对比策略成功，动态内容长度: {len(dynamic_content.strip())} 字符")
                        return dynamic_content.strip()
                except Exception as e:
                    self.logger.debug(f"基线对比策略失败: {str(e)}")

            # 【v3.6.25重构】精准提取 - 完整保留所有动态内容
            precision_script = """(() => {
                // ========== 配置：侧边栏区域标题（精确匹配） ==========
                const SIDEBAR_SECTION_TITLES = [
                    ...
                    '相关机构', '相关法规', '相关案例', '相关新闻',
                    '参考资料', '来源引用', '延伸阅读', '相关推荐',
                    'Related Events', 'Related Organizations',
                    'Related Companies', 'See Also', 'References'
                ];

                // 精确匹配：检查段落是否是侧边栏标题
                function isSidebarTitle(text) {
                ...
                // 检查元素是否在侧边栏中（基于DOM结构）
                function isSidebarElement(elem) {
                    const rect = elem.getBoundingClientRect();
                    const viewportWidth = window.innerWidth;
                    // 如果元素在页面右侧25%区域，很可能是侧边栏
                    if (rect.left > viewportWidth * 0.7) return true;
                    // 检查类名
                    const classes = (elem.className || '').toString().toLowerCase();
                    const sidebarClasses = ['sidebar', 'aside', 'related', 'recommend',
                                           'side-panel', 'right-panel', 'extra', 'panel'];
                    ...
                    while (parent && depth < 3) {
                        ...
                        for (const cls of sidebarClasses) {
                            if (parentClasses.includes(cls)) return true;
                        }
                        parent = parent.parentElement;
                        depth++;
                    }
                ...
                // 检查元素是否完全是侧边栏区域
                function isSidebarSection(elem) {
                    const text = (elem.innerText || elem.textContent || '').trim();
                    if (text.length === 0) return false;
                    const firstLine = text.split('\\n')[0].trim();
                    return isSidebarTitle(firstLine);
                }

                // 【v3.6.25新增】检查文本是否包含广告语关键词
                function containsAdKeywords(text) {
                    const adKeywords = ['总包学园', 'epcschool', '喝酒聊天', '1475'];
                ...
                // 【v3.6.25】策略A: 优先查找包含"总包学园"的完整回答容器
                // 这确保广告语完整保留
                const allContainers = document.querySelectorAll('div, section, article, main');
                for (const container of allContainers) {
                    if (isSidebarElement(container)) continue;
                    if (isSidebarSection(container)) continue;
                    if (container.querySelector('textarea, input[type="text"], [contenteditable="true"]')) continue;

                    const text = (container.innerText || container.textContent || '').trim();

                    // 【v3.6.25关键】如果包含广告语且长度足够，优先选择
                    if (containsAdKeywords(text) && text.length > 500) {
                        // 检查是否包含完整的AI回答结构
                        if (text.includes('第') && (text.includes('部分') || text.includes('节'))) {
                            // 这很可能是完整的AI回答
                ...
                if (!bestContent) {
                    ...
                    for (const selector of mainSelectors) {
                        try {
                            const elements = document.querySelectorAll(selector);
                            for (const elem of elements) {
                                if (isSidebarElement(elem)) continue;
                                if (isSidebarSection(elem)) continue;
                                if (elem.querySelector('textarea, input[type="text"], [contenteditable="true"]')) continue;
                ...
                // 【v3.6.25】策略C: 如果仍没找到包含广告语的内容，尝试更广泛的搜索
                if (!bestContent || !containsAdKeywords(bestContent)) {
                    const allDivs = document.querySelectorAll('div, section, article');
                    for (const div of allDivs) {
                        if (isSidebarElement(div)) continue;
                        if (isSidebarSection(div)) continue;
                        ...
                        if (text.length > 50 && text.length > bestLength) {
                            if (div.querySelector('textarea, input[type="text"], [contenteditable="true"]')) continue;
                            bestContent = text;
                            bestLength = text.length;
                        }
                    }
            ...
            try:
                content = await self.browser.page.evaluate(precision_script)
                # 【v3.6.31优化】移除长度限制，确保100%完整获取
                if content and content.strip():
                    self.logger.info(f"✅ 使用精准提取策略成功，长度: {len(content.strip())} 字符")
                    return content.strip()
            except Exception as e:
                self.logger.debug(f"精准提取策略失败: {str(e)}")

            # 【v3.6.14保留】策略1: 合并所有回答元素 - 作为回退
            merge_script = """(() => {
                // 收集所有可能的AI回答元素
                ...
                for (const selector of selectors) {
                    try {
                        ...
                        for (const elem of elements) {
                            ...
                            // 【v3.6.19新增】跳过包含排除关键词的内容
                            if (shouldExclude(text.substring(0, 100))) continue;
...
class WeChatDraftManager:
    ...
    def create_draft(self, title: str, content: str, author: str = None,
        ...
        cleaned_title = emoji_pattern.sub('', title)

        # 移除其他不允许的特殊字符，只保留中文、英文、数字、基本标点
        allowed_pattern = re.compile(
            r'[^\u4e00-\u9fff'  # 中文字符
            r'a-zA-Z0-9'        # 英文和数字
            r'！？，。、：；""''【】《》—…·'  # 中文标点
        ...
        cleaned_title = allowed_pattern.sub('', cleaned_title)
        cleaned_title = re.sub(r'\s+', ' ', cleaned_title).strip()

        if cleaned_title != title:
            self.logger.warning(f"⚠️ 标题包含特殊字符已自动清理: '{title}' -> '{cleaned_title}'")
            title = cleaned_title
...
class MarkdownToWeChat:
    ...
    def convert_answer_to_html(self, answer: str, question: str,
        ...
        # 【v3.6.44新增】CRITICAL: MD保存前完整性验证
        md_validation_result = self._validate_markdown_integrity(markdown_content, answer)
        if not md_validation_result['valid']:
            self.logger.error(f"❌ MD内容完整性验证失败: {md_validation_result['reason']}")
            # 尝试修复：使用原始answer重新生成
            self.logger.warning("⚠️ 尝试使用原始回答内容重新生成MD...")
            markdown_content = self._answer_to_markdown(
                answer, question,
                top_ad_image=self.config.top_ad_image,
                bottom_ad_image=self.config.bottom_ad_image,
            ...
            md_validation_result = self._validate_markdown_integrity(markdown_content, answer)
            if not md_validation_result['valid']:
                raise Exception(f"MD内容完整性验证失败（已重试）: {md_validation_result['reason']}")
            self.logger.info(f"✓ MD重新生成成功，完整性验证通过")
        ...
        # 【v3.6.37重要修改】广告图3次重试失败后直接删除，不再尝试本地路径
        # 原因：本地路径在微信公众号中无法显示，徒增复杂性
        if top_ad_url:
            top_ad_image = top_ad_url
            self.logger.info("✓ 顶部广告图上传成功，将插入文章")
        else:
            # 上传失败（已重试3次），不再插入顶部广告图
            top_ad_image = None
            self.logger.warning("⚠️ 顶部广告图上传失败（已重试3次），将跳过顶部广告图")

        if bottom_ad_url:
            bottom_ad_image = bottom_ad_url
            self.logger.info("✓ 底部广告图上传成功，将插入文章")
        else:
            # 上传失败（已重试3次），不再插入底部广告图
            bottom_ad_image = None
            self.logger.warning("⚠️ 底部广告图上传失败（已重试3次），将跳过底部广告图")
    ...
    def _validate_markdown_integrity(self, md_content: str, original_answer: str) -> dict:
        """【v3.6.44新增】验证Markdown内容完整性

        Args:
            md_content: 待验证的Markdown内容
            original_answer: 原始总包大脑回答

        Returns:
            dict: {'valid': bool, 'reason': str, 'score': float}
        """
        result = {'valid': True, 'reason': '', 'score': 1.0}

        # 1. 检查总包学园标记（必须有）
        integrity_markers = ['总包学园', 'epcschool.top', '中国首所工程总承包线上学校']
        has_marker = any(marker in md_content for marker in integrity_markers)
        if not has_marker:
            result['valid'] = False
            result['reason'] = f"缺少完整性标记（总包学园/epcschool.top）"
            result['score'] = 0.0
            self.logger.error(f"❌ MD完整性检查失败: {result['reason']}")
            return result

        # 2. 检查最小长度（至少500字符）
        min_length = 500
        if len(md_content) < min_length:
            result['valid'] = False
            result['reason'] = f"内容过短（{len(md_content)}字符 < {min_length}字符）"
            result['score'] = 0.3
            self.logger.error(f"❌ MD完整性检查失败: {result['reason']}")
            return result

        # 3. 检查内容覆盖率（MD内容应包含原文80%以上的文字）
        # 提取纯文字（去除广告图等HTML标签）
        import re
        md_text = re.sub(r'<[^>]+>', '', md_content)
        md_text = re.sub(r'\s+', '', md_text)

        # 从原始回答中提取纯文字
        original_text = re.sub(r'\s+', '', original_answer)

        # 计算覆盖率
        coverage_ratio = len(md_text) / len(original_text) if len(original_text) > 0 else 0
        # 【v3.6.47提高】从80%提高到90%，与HTML验证保持一致
        if coverage_ratio < 0.90:
            result['valid'] = False
            result['reason'] = f"内容覆盖率不足（{coverage_ratio:.1%} < 90%）"
            result['score'] = coverage_ratio
            self.logger.error(f"❌ MD完整性检查失败: {result['reason']}")
            return result

        # 4. 检查是否以完整句子结尾（检查最后一个非标点字符后是否有结束标点）
        stripped = md_content.strip()
        # 去除可能的HTML标签和广告图
        stripped = re.sub(r'</div>\s*$', '', stripped)
        ...
        if stripped:
            ...
            # 允许的结束字符
            valid_endings = '。！？.!?）)》」』"]\''
            if last_char not in valid_endings and not stripped.endswith('总包学园'):
                self.logger.warning(f"⚠️ MD可能未以完整句子结尾: '{last_char}'")
                # 不作为硬性失败，只是警告

        result['score'] = coverage_ratio
        self.logger.info(f"✓ MD完整性检查通过（覆盖率: {coverage_ratio:.1%}）")
        return result
...
"""
        ...
        # 【v3.6.51新增】确保正文不重复热点问题
        # 如果正文开头包含热点问题，移除它
        if question and structured_content:
            # 检查正文开头是否包含热点问题（精确匹配或高相似度）
            question_clean = question.strip()
            content_start = structured_content[:len(question_clean) + 50]  # 检查开头部分

            # 如果正文以热点问题开头，移除它
            if content_start.startswith(question_clean):
                self.logger.info("检测到正文开头重复热点问题，正在移除...")
                structured_content = structured_content[len(question_clean):].lstrip()
            elif question_clean in content_start[:len(question_clean) + 20]:
                # 允许少量前缀字符（如"问题："等）
                import re
                pattern = re.escape(question_clean)
                match = re.search(pattern, content_start[:len(question_clean) + 30])
                if match and match.start() < 20:  # 问题出现在前20个字符内
                    self.logger.info("检测到正文开头包含热点问题，正在移除...")
                    structured_content = structured_content[match.end():].lstrip()
...
{bottom_ad_md}"""
    ...
    def _structure_content_with_headings(self, content: str) -> str:
        ...
        try:
            ...
            # 【v3.6.52优化】增强版提示词：针对标题结构智能分析
            enhanced_system = """你是专业的EPC总承包领域排版编辑，擅长为技术文章添加清晰的结构化标题。

【核心任务】为EPC总承包领域的专业文章添加层次化的Markdown标题（## 和 ###）

【标题设置原则】
1. **层次分明**：先梳理文章逻辑，识别主要论点和次要论点
2. **逻辑清晰**：标题应反映文章的逻辑递进关系
3. **主题聚焦**：每个标题概括下方段落的核心主题
4. **简洁精准**：标题长度5-15字，不冗余

【标题层级规则】
- ## (二级标题)：用于主要章节/核心论点（如：问题分析、解决方案、关键要点）
- ### (三级标题)：用于子章节/细节要点（如：具体步骤、注意事项、案例说明）

【常见文章结构模板】
1. 问题分析类：问题背景 → 原因分析 → 解决方案 → 实施要点
2. 操作指南类：概述 → 准备工作 → 具体步骤 → 注意事项
3. 对比分析类：背景介绍 → 方案对比 → 优劣势分析 → 结论建议
4. 案例解析类：案例背景 → 关键问题 → 处理过程 → 经验总结

【严格规则 - 必须遵守】
1. 只输出添加了##和###标题后的原文内容，不添加任何其他文字
2. 绝对不能修改、删除、精简原文任何字、标点或格式
3. 绝对不能在输出中包含任何对话指令、说明或注释
4. 每篇文章至少添加3个二级标题，确保层次分明
5. 标题后紧跟相关段落，中间不要有空行
...
- 数量达标：至少3个二级标题"""
            ...
            enhanced_user = f"""请仔细阅读以下EPC总承包领域的专业文章，为其添加清晰的结构化标题。
...
========文章内容结束========"""
            ...
            for attempt in range(2):
                response = client.chat.completions.create(
                    ...
                    max_tokens=16000,  # 【v3.6.47提高】从10000提高到16000，支持更长的总包大脑回复
                    timeout=Constants.API_CALL_TIMEOUT  # 【v3.6.4修复】添加API调用超时
                )
    ...
    def _fallback_structure(self, content: str) -> str:
        """AI失败时的回退方法：基于规则添加简单标题结构

        【v3.6.52优化】Fallback时也添加标题，确保文章层次分明
        ...
        for i, para in enumerate(paragraphs):
            ...
            # 检测数字编号开头（如"1."、"一、"、"第一"等）
            import re
            number_match = re.match(r'^[一二三四五六七八九十]+[、．.]|^第[一二三四五六七八九十]+|^[\d]+[、．.]', para)
            if number_match:
                is_section_start = True
                section_title = para[:30]  # 取前30字符作为标题参考

            # 检测常见章节关键词
            section_keywords = ['首先', '其次', '再次', '最后', '总之', '综上所述',
                              '值得注意的是', '需要强调的是', '关键在于', '核心要点',
                              '建议', '解决方案', '具体措施', '实施步骤', '注意事项']

            for keyword in section_keywords:
                if para.startswith(keyword) and len(para) > 50:
                    is_section_start = True
                    break

            # 每3-4个段落添加一个二级标题（基于段落计数）
            if i > 0 and i % 4 == 0 and heading_count < 5:
                # 为中间段落添加标题
                if len(para) > 100:  # 只为较长的段落添加标题
                    # 尝试从段落中提取标题
            ...
            # 检测到的章节开头添加二级标题
            if is_section_start and heading_count < 6:
                # 提取标题文本
                title_text = section_title if section_title else para[:15]
                # 移除编号
                title_text = re.sub(r'^[一二三四五六七八九十]+[、．.]|^第[一二三四五六七八九十]+|^[\d]+[、．.]', '', title_text)
        ...
        self.logger.info(f"使用规则化标题结构（添加了{heading_count}个标题）")
        return result
    ...
    def _validate_html_quality(self, html_content: str, original_md: str, theme_name: str) -> float:
        ...
        try:
            ...
            # 【v3.6.44优化】精简占位符检测 - 只检测真正表示AI生成不完整的核心模式
            # 避免过度检测导致误判正常内容
            placeholder_patterns = [
                # HTML注释中的明确截断标记
                r'<!--\s*More content follows',
                r'<!--\s*Additional sections',
                r'<!--\s*content would be added',
                r'<!--\s*\[内容被截断\]',
                r'<!--\s*\[Content truncated\]',
                # 明确的TODO标记
                r'\[TODO\]', r'\[待补充\]', r'\[此处省略\]',
            ]
            for pattern in placeholder_patterns:
                if re.search(pattern, html_content, re.IGNORECASE):
                    self.logger.error(f"❌ CRITICAL: 检测到占位符注释，内容被截断！模式: {pattern}")
                    return 0.0  # 直接返回0分，强制重试

            # 【v3.6.44新增】CRITICAL: 检测AI截断消息
            # 当AI输出"由于篇幅限制"等消息时，表示内容被截断
            ai_truncation_patterns = [
                r'由于篇幅限制',
                r'仅展示了?部分',
                r'继续转换剩余',
                r'按照相同的样式规范继续',
            ...
            for pattern in ai_truncation_patterns:
                if re.search(pattern, html_content, re.IGNORECASE):
                    self.logger.error(f"❌ CRITICAL: 检测到AI截断消息！内容不完整！模式: {pattern}")
                    return 0.0  # 直接返回0分，强制重试

            # 【v3.6.36优化】精简占位符图片检测 - 只检测明确的占位符URL
            placeholder_image_patterns = [
                r'src=["\']图片URL["\']',  # 中文占位符URL
                r'src=["\']imageURL["\']',  # 英文占位符URL
                r'src=["\']图片["\']',  # 中文占位符
            ]
            for pattern in placeholder_image_patterns:
                if re.search(pattern, html_content, re.IGNORECASE):
                    self.logger.error(f"❌ CRITICAL: 检测到占位符图片标签！模式: {pattern}")
                    return 0.0  # 直接返回0分，强制重试

            # 【v3.6.32新增】CRITICAL: 检测总包大脑完整性标记
            zongbao_markers = ['总包学园', 'epcschool', 'EPCSchool', '喝酒聊天']
            has_marker = any(marker in html_content for marker in zongbao_markers)
            if not has_marker:
                self.logger.error("❌ CRITICAL: HTML中未检测到总包大脑完整性标记（总包学园等），内容不完整！")
                return 0.0  # 直接返回0分，强制重试
            ...
            # 【v3.6.37增强】更严格的内容长度检查 - 提高到90%
            # 原文内容必须100%保留，仅允许10%的格式差异
            content_ratio = len(text_only) / max(len(original_text), 1)
            if content_ratio < 0.90:
                self.logger.error(f"❌ CRITICAL: HTML内容长度不足！原文{len(original_text)}字符，HTML仅{len(text_only)}字符 (比例: {content_ratio:.1%})")
                return 0.0

            # 【v3.6.37新增】完整性警告 - 低于95%时警告
            if content_ratio < 0.95:
                self.logger.warning(f"⚠️ HTML内容略少于原文 ({content_ratio:.1%})，可能有轻微内容丢失")
            ...
            # 【v3.6.35修复】CRITICAL: 异常时返回0.0而非0.8，强制重试
            self.logger.error(f"HTML质量验证异常: {str(e)}，返回0分强制重试")
            return 0.0
    ...
    def _create_wechat_draft_directly(self, html_content: str, title: str,
        ...
        # 【v3.6.44新增】CRITICAL: 推送前最终完整性验证
        # 这是最后一道防线，确保不完整内容永远不会推送到公众号
        zongbao_markers = ['总包学园', 'epcschool', 'EPCSchool', '喝酒聊天']
        has_marker = any(marker in html_content for marker in zongbao_markers)
        if not has_marker:
            error_msg = "推送前验证失败: HTML内容缺少完整性标记（总包学园等），拒绝推送不完整内容！"
        ...
        import re
        ai_truncation_patterns = [
            r'由于篇幅限制', r'仅展示了?部分', r'继续转换剩余',
            r'按照相同的样式规范继续', r'剩余部分.*省略', r'篇幅所限',
            r'内容过长.*仅展示', r'篇幅原因', r'以上是部分转换',
        ]
        for pattern in ai_truncation_patterns:
            if re.search(pattern, html_content, re.IGNORECASE):
                error_msg = f"推送前验证失败: 检测到AI截断消息（{pattern}），拒绝推送不完整内容！"
                self.logger.error(f"❌ {error_msg}")
                raise Exception(error_msg)

        # 检查最小内容长度（至少500字符）
        if len(html_content) < 500:
            error_msg = f"推送前验证失败: HTML内容过短（{len(html_content)}字符 < 500字符），拒绝推送！"
            self.logger.error(f"❌ {error_msg}")
            raise Exception(error_msg)

        self.logger.info("✓ 推送前完整性验证通过，准备创建草稿...")
...
六、图片处理规则（重要）
═══════════════════════════════════════
【绝对禁止】在HTML中添加任何形式的图片：
- 禁止添加img标签
- 禁止使用src="图片URL"等占位符
- 禁止添加alt="描述"等占位文字
- 图片由系统在文章首尾自动添加（顶部广告图+底部广告图）
...
九、结尾处理（重要）
═══════════════════════════════════════
【绝对禁止】在HTML末尾添加任何签名、声明、备注，包括但不限于：
- "*本文由AI生成*"
- "*仅供参考*"
- 任何形式的免责声明
- 任何形式的来源标注
...
1. 所有CSS必须是内联样式（style属性）
2. 不使用外部样式表或<style>标签
3. 只使用安全的HTML标签：section, p, span, strong, em, br, h1-h6, blockquote, pre, code, div
4. 【禁止使用列表标签】绝对禁止使用ul、ol、li等列表标签，所有列表项必须转换为段落(p)格式，使用序号或符号作为段落开头（如"1. "、"• "、"◆ "等）
5. 【绝对禁止添加任何图片】不要添加任何img标签、占位符图片（如src="图片URL"）、装饰图片。图片由系统在首尾自动添加
...
6. 【绝对禁止】添加任何占位符图片标签（如src="图片URL"、alt="描述"、<!--图片占位符-->等）
7. 【绝对禁止】在正文中添加任何图片（包括占位符图片、装饰图片等）
8. 正文只保留文字内容，图片由系统自动在首尾添加（顶部广告图+底部广告图）
...
{self.config.prompt_html_user.format(theme_name=theme_name, md_content=processed_md_content)}"""
                ...
                if quality_score >= 0.95:
                    ...
                    self.logger.warning(f"尝试{attempt+1}: 排版质量{quality_score:.2%}，低于95%阈值")
                    if quality_score > best_quality_score:
                        best_html = html_content
                        best_quality_score = quality_score

            # 使用最佳结果（如果质量不达标，使用最佳尝试结果）
            # 【v3.6.51提高】阈值从0.85提高到0.90，确保更高质量
            if best_html and best_quality_score >= 0.90:
                html_content = best_html
                self.logger.info(f"使用最佳结果（质量评分: {best_quality_score:.2%}）")
            elif best_quality_score < 0.90:
                # 【v3.6.51更新】CRITICAL: 质量不达标时必须使用简单转换（阈值提高到90%）
                self.logger.warning(f"⚠️ AI排版质量不达标 ({best_quality_score:.2%} < 90%)，使用内置专业排版方法")
                return self._simple_convert(md_file)
            # 注意：不要重新获取response内容，否则会覆盖质量检查结果

            # 【v3.6.34新增】CRITICAL: 严格清理所有占位符图片（最后一道防线）
            # 定义占位符关键词列表
            placeholder_keywords = [
                '图片URL', '图片', 'imageURL', 'image_url', 'placeholder',
                '描述', 'description', '图片描述', 'alt_text', 'image_alt'
            ]

            # 第一步：移除所有包含占位符关键词的img标签
            def remove_placeholder_imgs(html):
                """移除所有包含占位符的img标签及其包装div"""
                import re

                # 先找出所有img标签
                img_pattern = r'<img[^>]*>'
                all_imgs = re.findall(img_pattern, html, re.IGNORECASE)
                ...
                for img_tag in all_imgs:
                    ...
                    for keyword in placeholder_keywords:
                        if keyword.lower() in img_tag.lower():
                            should_remove = True
                            break
                    ...
                    if should_remove:
                        ...
                        removed_count += 1
                        self.logger.info(f"🧹 移除占位符img: {img_tag[:80]}...")

                # 第二步：移除空的图片包装div（<div...><img...></div> 变成 <div...></div>）
                # 匹配只有空白内容的div
                empty_div_pattern = r'<div[^>]*text-align:\s*center[^>]*>\s*</div>'
                html = re.sub(empty_div_pattern, '', html, flags=re.IGNORECASE)

                # 第三步：移除图片占位符注释
                comment_patterns = [
                    r'<!--\s*图片占位符\s*-->',
                    r'<!--\s*Image placeholder\s*-->',
                    r'<!--\s*placeholder\s*-->',
                ]
            ...
            html_content, removed_count = remove_placeholder_imgs(html_content)
            if removed_count > 0:
                self.logger.info(f"✅ 总共清理了 {removed_count} 个占位符图片标签")

                # 【v3.6.35新增】清理后重新验证，确保没有遗漏
                # 【v3.6.47提高】阈值从70%提高到85%，确保更高质量
                revalidate_score = self._validate_html_quality(html_content, processed_md_content, theme_name)
                if revalidate_score < 0.85:
                    self.logger.error(f"❌ 清理占位符后质量不达标 ({revalidate_score:.2%} < 85%)，使用简单转换")
                    return self._simple_convert(md_file)
                self.logger.info(f"✓ 清理后重新验证通过 ({revalidate_score:.2%})")
            ...
            if ad_image_placeholders:
                ...
                # 【v3.6.28新增】跟踪已插入的广告图URL，防止重复
                inserted_ad_urls = set()
                for placeholder, ad_html in ad_image_placeholders:
                    if placeholder in html_content:
                        # 【v3.6.28新增】检查是否重复
                        import re as _re
                        url_match = _re.search(r'src="([^"]+)"', ad_html)
                        if url_match:
                            ad_url = url_match.group(1)
                            if ad_url in inserted_ad_urls:
                                self.logger.warning(f"⚠️ 检测到重复广告图，跳过: {ad_url[:50]}...")
                                html_content = html_content.replace(placeholder, '')
                                continue
                            inserted_ad_urls.add(ad_url)
                        ...
                        if "AD_IMG:0" in placeholder and ad_images:
                            ...
                            if url_match:
                                ad_url = url_match.group(1)
                                if ad_url in inserted_ad_urls:
                                    self.logger.warning(f"⚠️ 检测到重复广告图，跳过插入: {ad_url[:50]}...")
                                    continue
                                inserted_ad_urls.add(ad_url)
                        ...
                        # 如果是底部广告图，追加到末尾
                            ...
                            if url_match:
                                ad_url = url_match.group(1)
                                if ad_url in inserted_ad_urls:
                                    self.logger.warning(f"⚠️ 检测到重复广告图，跳过追加: {ad_url[:50]}...")
                                    continue
                                inserted_ad_urls.add(ad_url)
            ...
            if '429' in error_str or '速率限制' in error_str or 'rate limit' in error_str.lower():
                self.logger.warning(f"⚠️ 智谱AI速率限制，等待15秒后重试...")
                import time
                time.sleep(15)  # 【v3.6.54优化】从30s降为15s
                try:
                    # 重新尝试一次
    ...
    def _simple_convert(self, md_file: str) -> str:
        """内置的专业级Markdown转HTML转换（微信公众号兼容）

        生成带有完整内联CSS样式的HTML，确保微信公众号显示专业排版效果。
        支持主题颜色系统（秋日暖光等7种主题）。
        整合 baoyu-skills 专业排版方法 (v3.6.43)
        """
        self.logger.info("使用专业级内置方法转换（支持主题样式 + baoyu-skills优化）")

        try:
            with open(md_file, 'r', encoding='utf-8') as f:
                md_content = f.read()

            # 获取当前主题颜色 - 扩展颜色系统（借鉴baoyu-skills）
            theme_name = self.article_theme if hasattr(self, 'article_theme') else '秋日暖光'

            # 专业主题颜色系统 - 融合baoyu-skills配色方案
            theme_colors = {
                '秋日暖光': {
                    'primary': '#D97757',      # Warm Orange (baoyu modern默认)
                    'secondary': '#FFD700',
                    'accent': '#E4B1A0',       # baoyu accent
                    'text': '#3f3f3f',
                    'container_bg': 'rgba(250, 249, 245, 1)',  # baoyu modern背景
                    'blockquote_bg': 'rgba(255, 255, 255, 0.6)',
                },
                '春日清新': {
                    'primary': '#009874',      # Emerald Green (baoyu green)
                    'secondary': '#98FB98',
                    'accent': '#4CAF50',
                    'text': '#3f3f3f',
                    'container_bg': '#f0fff0',
                ...
                '深海静谧': {
                    'primary': '#0F4C81',      # Classic Blue (baoyu blue)
                    'secondary': '#87CEEB',
                    'accent': '#55C9EA',       # baoyu sky
                    'text': '#3f3f3f',
                    'container_bg': '#f0f8ff',
                ...
                '活力红': {
                    'primary': '#A93226',      # China Red (baoyu red)
                    'secondary': '#FF6B6B',
                    'accent': '#FA5151',       # baoyu vermilion
                    'text': '#3f3f3f',
                    'container_bg': '#fff5f5',
                    'blockquote_bg': '#ffebee',
                },
                '简约蓝': {
                    'primary': '#4682B4',
                    'secondary': '#B0C4DE',
            ...
            colors = theme_colors.get(theme_name, theme_colors['秋日暖光'])

            # 微信公众号兼容的CSS样式 - 融合baoyu-skills专业排版
            # 字体使用baoyu-skills推荐的中文字体栈
            ...
            container_style = (
                ...
                f'font-weight: 400; '
                f'color: {colors["text"]}; '
                f'background-color: {colors["container_bg"]}; '
                f'word-wrap: break-word; '
                f'border-radius: 25px; '
                f'border: 1px solid rgba(255, 255, 255, 0.01);'
            )

            # H1 - 药丸形标题 (baoyu modern风格)
            h1_style = (
                f'display: table; '
                f'padding: 0.3em 1em; '
                f'margin: 20px auto; '
            ...
            # H2 - 底部边框标题 (baoyu modern风格)
            h2_style = (
                f'display: block; '
                f'padding: 0.2em 0; '
                f'padding-bottom: 0; '
                f'margin: 0 auto 20px; '
                f'width: 100%; '
                f'color: {colors["primary"]}; '
                f'font-size: 20px; '
                f'font-weight: bold; '
                f'letter-spacing: 0.578px; '
                f'line-height: 1.7; '
            ...
            # 引用块样式 - baoyu modern风格
            blockquote_style = (
                f'font-style: normal; '
                f'padding: 15px 0; '
                f'margin: 12px 0; '
                f'border-left: 7px solid {colors["accent"]}; '
                f'border-radius: 10px; '
                f'color: {colors["text"]}; '
                f'background-color: {colors["blockquote_bg"]};'
            )
            ...
            code_inline_style = (
                f'font-size: 90%; '
                f'color: #d14; '
                f'background: rgba(27, 31, 35, 0.05); '
                f'padding: 3px 5px; '
                f'border-radius: 4px;'
            )
            ...
            code_block_style = (
                ...
                f'margin: 10px 8px; '
                f'background: #f5f5f5; '
                f'box-shadow: inset 0 0 10px rgba(0,0,0,0.05);'
            )

            # 开始转换
            html = md_content

            # 1. 处理代码块（先处理，避免被其他规则影响）- baoyu风格
            ...
            html = re.sub(r'```(\w*)\n(.*?)```',
                         lambda m: f'<pre class="code__pre" style="{code_block_pre_style}"><code>{m.group(2)}</code></pre>',
            ...
            # 3. 处理标题（从高到低，避免误匹配）
            html = re.sub(r'^### (.+)$',
                         lambda m: f'<h3 style="{h3_style}">{m.group(1)}</h3>',
                         html, flags=re.MULTILINE)
            html = re.sub(r'^## (.+)$',
                         lambda m: f'<h2 style="{h2_style}">{m.group(1)}</h2>',
                         html, flags=re.MULTILINE)
            html = re.sub(r'^# (.+)$',
                         lambda m: f'<h1 style="{h1_style}">{m.group(1)}</h1>',
                         html, flags=re.MULTILINE)
            ...
            def process_blockquote(text):
                ...
                for line in lines:
                    if line.startswith('> '):
                        ...
                        if in_blockquote:
                            quote_text = '<br/>'.join(quote_content)
                            result.append(f'<blockquote style="{blockquote_style}">{quote_text}</blockquote>')
                            quote_content = []
                            in_blockquote = False
                        result.append(line)
            ...
            def process_list(text):
                ...
                for line in lines:
                    ...
                    if stripped.startswith('- ') or stripped.startswith('* '):
                        ...
                        if not in_list:
                            in_list = True
                        list_items.append(stripped.split('. ', 1)[1] if '. ' in stripped else stripped)
                    else:
                        if in_list and list_items:
                            items_html = ''.join([f'<li style="{li_style}">{item}</li>' for item in list_items])
                            result.append(f'<ul style="{list_style}">{items_html}</ul>')
                            list_items = []
                            in_list = False
                        result.append(line)
            ...
            html = re.sub(r'`(.+?)`',
                         lambda m: f'<code style="{code_inline_style}">{m.group(1)}</code>',
                         html)
            ...
            for p in paragraphs:
                ...
                # 检查是否已经是块级元素
                block_patterns = [
                    r'^<h[1-6]', r'^<ul', r'^<ol', r'^<blockquote',
                    r'^<pre', r'^<div', r'^<hr', r'^<img', r'^<p'
                ]

                is_block = any(re.match(pattern, p) for pattern in block_patterns)
                ...
                if is_block:
                    ...
                    if len(lines) > 1:
                        ...
                        if not any(line.strip().startswith(('- ', '* ', '>')) for line in lines):
                            p = '<br/>'.join(lines)
                    processed_paragraphs.append(f'<p style="{p_style}">{p}</p>')
            ...
            # 10. 包装在容器中
            final_html = f'<section style="{container_style}">\n{html}\n</section>'

            self.logger.info(f"专业级转换完成，主题: {theme_name}")
            return final_html

        except Exception as e:
            self.logger.error(f"专业级转换失败: {str(e)}")
            # 【v3.6.45修复】即使失败也必须返回有专业排版的内容，不能直接返回原始Markdown
            import traceback
            self.logger.debug(traceback.format_exc())
            try:
                with open(md_file, 'r', encoding='utf-8') as f:
                ...
                # 使用最基本的Markdown转HTML转换，确保有排版
                self.logger.warning("使用最基本的Markdown转HTML转换作为最终fallback")

                # 基本转换：至少确保段落、标题有样式
                html = md_content
                html = re.sub(r'^# (.+)$', r'<h1 style="font-size:24px;font-weight:bold;margin:20px 0;border-bottom:2px solid #D97757;padding-bottom:10px;">\1</h1>', html, flags=re.MULTILINE)
                html = re.sub(r'^## (.+)$', r'<h2 style="font-size:20px;font-weight:bold;margin:16px 0;color:#D97757;">\1</h2>', html, flags=re.MULTILINE)
                html = re.sub(r'^### (.+)$', r'<h3 style="font-size:18px;font-weight:bold;margin:14px 0;border-left:4px solid #D97757;padding-left:10px;">\1</h3>', html, flags=re.MULTILINE)
                html = re.sub(r'\*\*(.+?)\*\*', r'<strong style="color:#D97757;font-weight:bold;">\1</strong>', html)
                html = re.sub(r'\n\n', r'</p><p style="margin:12px 0;line-height:1.8;text-indent:2em;">', html)
                html = re.sub(r'\n', r'<br/>', html)

                # 包装在专业容器中
                return f'''<section style="max-width:100%;padding:15px;font-family:-apple-system-font,BlinkMacSystemFont,'Helvetica Neue','PingFang SC','Hiragino Sans GB','Microsoft YaHei',Arial,sans-serif;font-size:15px;line-height:1.8;color:#3f3f3f;background-color:#faf9f5;">
<p style="margin:12px 0;line-height:1.8;text-indent:2em;">{html}</p>
</section>'''
            except Exception as inner_e:
                self.logger.error(f"最终fallback也失败: {str(inner_e)}")
                # 绝对最后的fallback：至少确保内容被正确包装
                return f'<section style="padding:15px;font-size:15px;line-height:1.8;color:#3f3f3f;"><p>{md_content}</p></section>'
...
def main():
    ...
    parser.add_argument('-n', '--night', action='store_true',
                       help='生产模式：依次使用所有关键词生成文章，每个关键词间隔5分钟')
    parser.add_argument('--daily-19', action='store_true',
                       help='每日19点定时模式：每天19:00自动启动晚上模式，发送企业微信通知（v3.6.26新增）')
    parser.add_argument('--daily-hour', type=int, default=19,
                       help='自定义每日定时启动时间（小时，默认19）')
    ...
    # 输出配置信息
    logger.info("=" * 60)
    logger.info("ZBBrain-Write v3.6.54 (运行效率大幅优化)")
    logger.info("=" * 60)
    ...
    # 确定运行模式
    # 【v3.6.26更新】新增每日19点定时模式
    # 优先级: --once > --daily-19 > --schedule > 默认晚上模式
    if args.once:
        use_schedule = False
        use_night_mode = False
        use_daily_night = False
        logger.info("⏺ 命令行参数 --once: 仅运行一次")
    elif args.daily_19:
        use_schedule = False
        use_night_mode = False
        use_daily_night = True
        logger.info(f"🕖 命令行参数 --daily-19: 每日{args.daily_hour:02d}:00定时模式")
    elif args.schedule:
        use_schedule = True
        use_night_mode = False
        use_daily_night = False
        logger.info("⏺ 命令行参数 --schedule: 白天定时模式")
    else:
        # 默认使用生产模式
        use_schedule = False
        use_night_mode = True
        use_daily_night = False
        logger.info("🔄 默认模式: 生产模式")

    if use_daily_night:
        # 每日19点定时模式：每天固定时间启动晚上模式
        logger.info(f"🕖 启动每日定时晚上模式")
        logger.info(f"   每天启动时间: {args.daily_hour:02d}:00 (北京时间)")
        logger.info(f"   发送企业微信通知: 启动时 + 每篇文章完成后")
        run_daily_night_scheduler(temp_config, logger, args.pages, args.mode, args.send_email, args.daily_hour)
    elif use_night_mode:
        # 生产模式：依次使用所有关键词生成文章
        logger.info("🔄 启动生产模式")
        logger.info("   规则：依次使用所有关键词，每个间隔5分钟")
        logger.info("   停止条件：关键词用完")
        run_night_mode(temp_config, logger, args.pages, args.mode, args.send_email)
    elif use_schedule:
        logger.info("🕐 启动白天定时模式")
        logger.info(f"   运行间隔: {temp_config.run_frequency_minutes} 分钟")
...
def run_night_mode(config: Config, logger: Logger, max_pages: int,
                   mode: str, send_email: bool):
    """运行晚上模式

    【v3.6.17新增】晚上模式运行规则：
    1. 项目启动时立即用第一个关键词运行
    2. 完成一篇文章后，间隔5分钟用下一个关键词
    3. 全部关键词用完就停止
    4. 或者最晚运行时间不超过23:59

    【v3.6.48修复】使用关键词轮换机制，从上次停止的关键词继续
    """
    import signal
    import time
    import pytz
    ...
    # 【v3.6.48修复】使用配置中的关键词列表和轮换状态
    keywords = config.get_topic_keywords()

    if not keywords:
        logger.error("关键词列表为空，无法运行")
        return

    # 获取当前轮换索引，从上次停止的地方继续
    start_index = config.current_keyword_index if hasattr(config, 'current_keyword_index') else 0

    # 如果索引超出范围，重置为0
    if start_index >= len(keywords):
        start_index = 0
        logger.info("🔄 关键词索引已重置为0")

    logger.info("=" * 60)
    logger.info("🌙 晚上模式调度器已启动 (v3.6.48)")
    logger.info(f"📋 关键词总数: {len(keywords)}")
    logger.info(f"📌 起始索引: {start_index} ({keywords[start_index]})")
    logger.info(f"⏰ 最晚停止时间: 23:59")
    logger.info(f"🔄 关键词间隔: 10 分钟")
    logger.info("=" * 60)
    ...
    # 【v3.6.48修复】使用轮换机制，从start_index开始循环
    current_idx = start_index
    keywords_processed = 0
    total_keywords = len(keywords)

    while keywords_processed < total_keywords and stop_flag['running']:
        keyword = keywords[current_idx]

        now_beijing = datetime.now(beijing_tz)

        logger.info("=" * 60)
        logger.info(f"🔄 生产模式 - 关键词 {keywords_processed + 1}/{total_keywords}: {keyword} (索引: {current_idx})")
        ...
        try:
            ...
            success = task.run(keyword, max_pages, mode, None, send_email)

            if success:
                success_count += 1
                # 尝试获取文章标题
                if hasattr(task, 'last_article_title') and task.last_article_title:
                    article_titles.append(f"✅ {keyword}: {task.last_article_title}")
                else:
                    article_titles.append(f"✅ {keyword}")
            else:
                fail_count += 1
                article_titles.append(f"❌ {keyword}: 运行失败")

        except Exception as e:
            logger.error(f"关键词 '{keyword}' 运行出错: {str(e)}")
            fail_count += 1
            article_titles.append(f"❌ {keyword}: {str(e)}")

        # 【v3.6.48修复】更新轮换状态
        next_idx = (current_idx + 1) % len(keywords)
        config._save_keyword_rotation_state({
        ...
        # 如果不是最后一个关键词，等待2分钟
        if keywords_processed < total_keywords and stop_flag['running']:
            next_keyword = keywords[current_idx]
            logger.info(f"⏳ 等待 2 分钟后使用下一个关键词...")
            logger.info(f"⏭️ 下一个关键词: {next_keyword} (索引: {current_idx})")

            # 【v3.6.54优化】分段等待，每30秒检查一次（原5分钟降为2分钟）
            waited = 0
            wait_seconds = 2 * 60  # 2分钟
            while waited < wait_seconds and stop_flag['running']:
                ...
                if waited > 0 and waited % 60 == 0:
                    logger.info(f"⏳ 等待中... 已等待 {waited // 60} 分钟")

    # 输出总结
    logger.info("=" * 60)
    logger.info("🔄 生产模式运行结束")
    logger.info("=" * 60)
    logger.info(f"📊 运行统计:")
    logger.info(f"   ✅ 成功: {success_count}")
    logger.info(f"   ❌ 失败: {fail_count}")
    logger.info(f"   📝 总计: {success_count + fail_count}/{len(keywords)}")
    logger.info("")
    logger.info("📋 文章列表:")
    ...
    try:
        ...
        summary_msg = f"""🌙 **晚上模式运行完成**

📊 **运行统计**:
- ✅ 成功: {success_count}
- ❌ 失败: {fail_count}
- 📝 总计: {success_count + fail_count}/{len(keywords)}

📋 **文章列表**:
{chr(10).join(article_titles)}

---
_总包大脑自动写作系统 v3.6.19_"""
        notifier.send_custom_message(summary_msg)
    except Exception as e:
        logger.error(f"发送完成通知失败: {str(e)}")

    # 【v3.6.54】运行完成后重置人设为"工程专家版"
    try:
        prompts_data = config._load_prompts_rotation_config() if hasattr(config, '_load_prompts_rotation_config') else {}
        if prompts_data and 'prompts' in prompts_data:
            for i, p in enumerate(prompts_data['prompts']):
                if '工程专家' in p.get('name', ''):
                    config.current_prompt_index = i
                    config._save_prompts_rotation_state()
                    logger.info(f"✅ 已重置回复人设为: {p['name']} (索引: {i})")
                    break
    except Exception as e:
        logger.warning(f"重置回复人设失败: {e}")

    logger.info("🛑 晚上模式调度器已停止")


def run_daily_night_scheduler(config: Config, logger: Logger, max_pages: int,
                              mode: str, send_email: bool, target_hour: int = 19):
    """每天指定时间启动晚上模式（v3.6.26 新增）

    功能说明：
    - 每天在指定时间（默认19:00）自动启动
    - 启动时发送企业微信通知
    - 依次使用所有关键词生成文章
    - 每篇文章完成后发送企业微信通知
    - 完成后等待到第二天同一时间再次运行
    ...
    logger.info("=" * 60)
    logger.info(f"🕖 每日定时晚上模式调度器已启动 (v3.6.26)")
    logger.info(f"⏰ 每天启动时间: {target_hour:02d}:00 (北京时间)")
    logger.info("=" * 60)

    notifier = NotificationManager(config, logger)

    def calculate_wait_seconds() -> int:
        """计算到下一个目标时间的等待秒数"""
        now = datetime.now(beijing_tz)
        ...
        return int(wait_seconds), target_time

    def send_startup_notification(target_time: datetime):
        """发送启动通知"""
        try:
            msg = f"""🕖 **每日定时晚上模式启动**

⏰ **启动时间**: {target_time.strftime('%Y-%m-%d %H:%M:%S')}
📅 **运行模式**: 晚上模式（依次使用所有关键词）
🔄 **下次运行**: 明天 {target_hour:02d}:00

---
_总包大脑自动写作系统 v3.6.26_"""
            notifier.send_custom_message(msg)
            logger.info("✓ 启动通知已发送到企业微信")
        except Exception as e:
            logger.error(f"发送启动通知失败: {str(e)}")

    def send_article_notification(article_title: str, keyword: str, index: int, total: int, success: bool):
        """发送文章完成通知"""
        try:
            status_icon = "✅" if success else "❌"
            status_text = "发布成功" if success else "发布失败"
            msg = f"""📝 **文章{status_text}**
...
_总包大脑自动写作系统 v3.6.26_"""
            notifier.send_custom_message(msg)
            logger.info(f"✓ 文章通知已发送: {article_title[:20]}...")
        except Exception as e:
            logger.error(f"发送文章通知失败: {str(e)}")

    def send_daily_summary(success_count: int, fail_count: int, article_titles: list):
        """发送每日运行总结"""
        try:
            total = success_count + fail_count
            msg = f"""📊 **每日运行完成**

⏰ **完成时间**: {datetime.now(beijing_tz).strftime('%Y-%m-%d %H:%M:%S')}
...
📋 **今日文章**:
{chr(10).join(['• ' + t[:30] + ('...' if len(t) > 30 else '') for t in article_titles[:10]])}
{f'_...还有 {len(article_titles) - 10} 篇_' if len(article_titles) > 10 else ''}
...
_总包大脑自动写作系统 v3.6.26_"""
            notifier.send_custom_message(msg)
            logger.info("✓ 每日总结已发送到企业微信")
        except Exception as e:
            logger.error(f"发送每日总结失败: {str(e)}")

    # 主循环
    while stop_flag['running']:
        try:
            # 计算等待时间
            wait_seconds, target_time = calculate_wait_seconds()
            wait_hours = wait_seconds / 3600

            logger.info(f"⏳ 等待到 {target_time.strftime('%Y-%m-%d %H:%M:%S')} (还需等待 {wait_hours:.1f} 小时)")

            # 分段等待，每5分钟检查一次停止标志
            check_interval = 300  # 5分钟
            elapsed = 0
            while elapsed < wait_seconds and stop_flag['running']:
                sleep_time = min(check_interval, wait_seconds - elapsed)
                time.sleep(sleep_time)
                elapsed += sleep_time

                # 每小时报告一次等待状态
                if elapsed % 3600 < check_interval:
                    remaining = (wait_seconds - elapsed) / 3600
                    logger.info(f"⏳ 等待中... 剩余 {remaining:.1f} 小时")
            ...
            # ========== 到达目标时间，开始运行 ==========
            logger.info("=" * 60)
            logger.info(f"🕖 到达定时启动时间: {target_time.strftime('%Y-%m-%d %H:%M:%S')}")
            logger.info("=" * 60)

            # 发送启动通知
            send_startup_notification(target_time)

            # 获取关键词列表
            keywords = config.get_topic_keywords()
            if not keywords:
                logger.warning("未找到关键词配置，跳过本次运行")
                time.sleep(3600)
                continue
            ...
            # 【v3.6.48修复】使用轮换机制，从上次停止的位置继续
            start_idx = config.current_keyword_index if hasattr(config, 'current_keyword_index') else 0
            if start_idx >= len(keywords):
                start_idx = 0
            logger.info(f"📌 从索引 {start_idx} 开始（轮换机制）")
            ...
            while keywords_processed < total_keywords and stop_flag['running']:
                ...
                if now.hour >= 23 and now.minute >= 59:
                    logger.info("⏰ 已到23:59，停止今日运行")
                    break

                logger.info("=" * 60)
                logger.info(f"📝 关键词 {keywords_processed + 1}/{total_keywords}: {keyword} (索引: {current_idx})")
                logger.info(f"📅 当前时间: {now.strftime('%Y-%m-%d %H:%M:%S')}")
                logger.info("=" * 60)

                # 重新加载配置（确保获取最新的公众号和提示词）
                fresh_config = Config(config.config_file)

                try:
                    # 执行一次运行
                    result = asyncio.run(run_once(
                        fresh_config, logger, keyword, max_pages, mode, None, send_email
                    ))
                    ...
                    if result and result.get('success'):
                        ...
                        # 发送文章成功通知
                        send_article_notification(title, keyword, keywords_processed + 1, total_keywords, True)
                        logger.info(f"✅ 关键词 '{keyword}' 处理成功")
                    else:
                        fail_count += 1
                        error_msg = result.get('error', '未知错误') if result else '运行失败'
                        # 发送文章失败通知
                        send_article_notification(error_msg, keyword, keywords_processed + 1, total_keywords, False)
                        logger.error(f"❌ 关键词 '{keyword}' 处理失败: {error_msg}")

                except Exception as e:
                    fail_count += 1
                    logger.error(f"❌ 关键词 '{keyword}' 执行出错: {str(e)}")
                    send_article_notification(str(e)[:50], keyword, keywords_processed + 1, total_keywords, False)
                ...
                # 【v3.6.54优化】关键词之间间隔2分钟（原5分钟）
                if keywords_processed < total_keywords and stop_flag['running']:
                    logger.info(f"⏳ 等待2分钟后处理下一个关键词 (下一个: {keywords[current_idx]})...")
                    for _ in range(12):  # 2分钟 = 120秒 / 10秒
                        if not stop_flag['running']:
                            break
                        time.sleep(10)

            # 发送每日总结
            send_daily_summary(success_count, fail_count, article_titles)

            logger.info("=" * 60)
            logger.info(f"📊 今日运行完成 - 成功: {success_count}, 失败: {fail_count}")
            logger.info("=" * 60)

        except Exception as e:
            logger.error(f"❌ 每日运行出错: {str(e)}")
            # 发送错误通知
            try:
                error_msg = f"""⚠️ **每日运行出错**
...
---
_总包大脑自动写作系统 v3.6.26_"""
                notifier.send_custom_message(error_msg)
            except Exception as e:
                logger.warning(f"发送错误通知失败: {e}")
            # 【v3.6.54优化】等待10分钟后重试（原1小时）
            time.sleep(600)

    logger.info("🛑 每日定时晚上模式调度器已停止")
...
def run_scheduler(config: Config, logger: Logger, keyword: str, max_pages: int,
    ...
    logger.info(f"🔄 运行频率: 每{config.run_frequency_minutes}分钟")
...
_总包大脑自动写作系统 v3.5.0_"""
                    ...
                    next_run = current_time_for_next_run + timedelta(minutes=config.run_frequency_minutes)
                    stats_tracker.set_next_run(next_run)
                    logger.info(f"⏭️ 下次运行时间: {next_run.strftime('%Y-%m-%d %H:%M:%S')} (北京时间)")

                    # 等待到下次运行时间
                    wait_seconds = config.run_frequency_minutes * 60
                    logger.info(f"😴 等待 {config.run_frequency_minutes} 分钟后下次运行...")
